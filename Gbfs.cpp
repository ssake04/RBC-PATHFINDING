...
struct Node
{
    ...
    Node() : x(0), y(0), h(0), parent(nullptr) {}

    Node(int x, int y, int h, Node *parent = nullptr) 
        : x(x), y(y), h(h), parent(parent) {}
...
int findPath(char grid[50][50], int startX, int startY, int endX, int endY, pair<int, int> path[], int &pathLength) 
{
    ...
    priority_queue<Node, vector<Node>, greater<Node>> openList; // Use priority queue for GBFS

    openList.push(Node(startX, startY, directionface(startX, startY, endX, endY)));
    ...
    while (!openList.empty()) 
    {
        ...
        for (int i = 0; i < 8; ++i) 
        {
            ...
            {
                openList.push(Node(nx, ny, directionface(nx, ny, endX, endY), new Node(current))); 
            }
        }
    }
    return 0;
